...
public class LockScreenActivity extends AppCompatActivity {
    ...
    //Plays random notes got from xml file "notes.xml" in "src/main/assets" folder. Notes are stored in "res/raw" folder
    private void play() {
    ...
    private void unlock() {
        if (selectedNotesList.containsAll(outputtedNotesList) && outputtedNotesList.containsAll(selectedNotesList))
            ...
            //Colors the buttons of the wrong notes:
            //green if the entry is correct,
            //orange if the entry is missing
                ...
                switch (snl) {
                    case "do":
                        if (outputtedNotesList.contains(snl)) {
                            ...
                            notesColor.add(Color.parseColor("#91EA2A"));
                        } else {
                            buttonDo.setBackground(ContextCompat.getDrawable(this, R.drawable.wrong_round));
                            notesColor.add(Color.parseColor("#FFC12A"));
                        }
                        break;
                    case "dodie":
                        if (outputtedNotesList.contains(snl)) {
            ...
            //Colors the outputted notes in the textview:
            //green if the entry is correct,
            //red if the entry is incorrect.
            //Colors the buttons of red if the entry is wrong
