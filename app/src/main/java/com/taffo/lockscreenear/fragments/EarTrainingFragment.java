...
public final class EarTrainingFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.ear_training_activity, container, false);
        ...
        if (easterEggChallenge)
            rootView.setBackground(ContextCompat.getDrawable(mContext, R.mipmap.easter_egg_wallpaper));
    ...
    public void onPause() {
        ...
        if (isRemoving()) {
            for (MediaPlayer mPlayer : mediaPlayer)
                mPlayer.release();
            activity.getWindow().setStatusBarColor(mContext.getColor(R.color.custom_launcher_background_green));
            actionBar.setBackgroundDrawable(new ColorDrawable(mContext.getColor(R.color.custom_launcher_background_green)));
            activity.getWindow().getDecorView().setBackground(ContextCompat.getDrawable(mContext, R.color.white));
    ...
    private void resetAndRestart() {
        ...
        prepareToPlay();
        if (easterEggChallenge)
            requireView().setBackground(ContextCompat.getDrawable(mContext, R.mipmap.easter_egg_wallpaper));
        else
            requireView().setBackground(ContextCompat.getDrawable(mContext, R.color.custom_background_ear_training_deep_blue));
    }

    private void easterEggChallenge() {
        if (!sp.getSharedmPrefEasterEggChallengeCompleted() && sp.getSharedmPrefEasterEggChallengeStarted()) {
            ...
            if (new Random().nextInt(sp.getSharedmPrefEasterEggChallengeGuesses()) + 1 == 1) {
                ...
                textViewNotes.setText(getString(R.string.easter_egg_challenge_started_title));
                textViewNotes.setTextColor(mContext.getColor(R.color.easter_egg_gold));
        ...
        if (selectedNotesList.size() >= UNIQUE_NOTES) {
            if (selectedNotesList.containsAll(outputtedNotesList) && outputtedNotesList.containsAll(selectedNotesList)) {
                ...
                if (!sp.getSharedmPrefEasterEggChallengeCompleted() && sp.getSharedmPrefEasterEggChallengeStarted()) {
                    ...
                    if (easterEggChallenge) {
                        sp.setSharedmPrefEasterEggChallengeCompleted(true);
                        textViewNotes.setTextColor(mContext.getColor(R.color.easter_egg_gold));
